...
export default ({ config }: ConfigContext): ExpoConfig => {
  ...
  return {
    ...
    ios: {
      ...
      supportsTablet: true,
      bundleIdentifier: process.env.BILT_IOS_BUNDLE_ID ?? 'me.bilt.cyclewise',
    },
    android: {
      package: process.env.BILT_ANDROID_PACKAGE ?? 'me.bilt.cyclewise',
    },
    web: {
      bundler: 'metro',
    ...
    plugins: ['expo-router', 'expo-font', 'expo-secure-store', 'expo-sharing', ...nativePlugins],
    experiments: {
      typedRoutes: true,
      reactCompiler: true,
    },
